...
/**
 * Konvertiert einen String-Key in ein Krypto-Key-Objekt für AES-GCM
 */
...
/**
 * Shuffling (Erzeugt den geshuffelten Teil)
 */
...
/**
 * Unshuffling (Rekonstruiert Daten aus dem geshuffelten Teil)
 */
...
/**
 * Verschlüsselt Text und formatiert das Ergebnis zu [config-block][geshuffelter-teil]
 * @param {string} text - Der zu verschlüsselnde Klartext
 * @param {string} key - Der 32-stellige AES-Key
 * @param {string} config - Der 2-zeilige (16-Bit) Config-Block
 * @returns {Promise<string>}
 */
export async function encoder(text, key, config) {
    if (config.length !== 2) throw new Error("Config muss exakt 16-Bit (2 Zeichen) lang sein.");
    
    const encryptedBody = await aesEncrypt(text, key);
    const shuffledPart = shuffleData(key, encryptedBody);
    
    return `${config}${shuffledPart}`;
}
 ...
 * @returns {Promise<{config: string, text: string}>}
 */
